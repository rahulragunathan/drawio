...
graph = ET.SubElement(diagram, "mxGraphModel",
                      dx="1200", dy="700", grid="0", gridSize="10",
...
container(40, 92, 320, 496, "External Clients",
          stroke="#3a6ea5", fill="#eaf3fb", fontColor="#1f3864",
          fill_dark="#10233a", fontColor_dark="#cfe2ff")
container(380, 92, 500, 496, "Application Tier",
          stroke="#5b3fbf", fill="#f3f0ff",
          fill_dark="#1c1633", stroke_dark="#b59dff", fontColor_dark="#d9ccff")
container(900, 92, 340, 496, "Data Tier",
          stroke="#003c71", fill="#e7f0fb",
          fill_dark="#0c1f33", stroke_dark="#8fb8e6", fontColor_dark="#bfe0ff")


# ----- External clients -----
browser = box(70, 140, 260, 80,
              "<b>Browser / Mobile App</b><br>"
              + sub("User-Facing Clients"),
              fill=COLOR_PRIMARY_BLUE, fill_dark=COLOR_PRIMARY_BLUE_DARK,
...
cdn_future = box(70, 248, 260, 96,
                 "<b>CDN</b><br>" + sub("Edge Caching", size=10) + "<br>"
                 + desc("Future: geographic edge distribution beyond the "
                        "initial single-region deploy.", size=10),
                 fill="#e0e0e0", stroke=COLOR_FUTURE_GREY,
                 fontColor="#616161", fontSize=11, bold=False, dashed=True)

config = box(70, 360, 260, 200,
             "<b>Configuration (Git Repo)</b><br><br>"
             + desc("• Service routing rules<br>"
                    "• Feature flags<br>"
                    "• Connection strings<br>"
                    "• Rate-limit policies<br>"
                    "• Secret references"),
             fill="#fff4d6", stroke=COLOR_CONFIG_GOLD,
             fontColor="#7f6000", fontSize=11,
             fill_dark="#3a2f00", stroke_dark=COLOR_CONFIG_GOLD_DARK,
             fontColor_dark="#ffe9a6",
             valign="top", halign="left",
             spacingTop=10, spacingLeft=10, bold=False)


# ----- Application tier (two rows, ~100px routing band between) -----
api_gw = box(410, 140, 200, 80,
             "<b>API Gateway</b><br>" + sub("Entry Point"),
             fill=COLOR_GATEWAY_ORANGE, fontSize=12, bold=False)
auth = box(650, 140, 210, 80,
           "<b>Auth Service</b><br>" + sub("Token Verification"),
           fill=COLOR_GATEWAY_ORANGE, fontSize=12, bold=False)
web = box(410, 320, 200, 80,
          "<b>Web Service</b><br>" + sub("Business Logic"),
          fill=COLOR_FRONTEND_GREEN, fontSize=12, bold=False)
worker = box(650, 320, 210, 80,
             "<b>Background Worker</b><br>" + sub("Async Jobs"),
             fill=COLOR_WORKER_PURPLE, stroke=COLOR_ORCH_PURPLE,
             fill_dark=COLOR_WORKER_PURPLE_DARK,
             stroke_dark=COLOR_ORCH_PURPLE_DARK, fontColor_dark="#2a1f4d",
             fontSize=12, bold=False)


# ----- Data tier -----
primary_db = box(920, 140, 290, 104,
                 "<b>Primary Database</b><br>"
                 + sub("Source of Truth") + "<br>"
                 + desc("Transactional reads and writes"),
                 fill=COLOR_DATASTORE_NAVY, fill_dark=COLOR_DATASTORE_NAVY_DARK,
                 fontColor_dark="#0a2540", fontSize=12, bold=False)
cache = box(920, 272, 290, 72,
            "<b>Cache</b><br>" + sub("Hot Reads"),
            fill=COLOR_DATASTORE_NAVY, fill_dark=COLOR_DATASTORE_NAVY_DARK,
            fontColor_dark="#0a2540", fontSize=12, bold=False)
queue = box(920, 372, 290, 72,
            "<b>Message Queue</b><br>" + sub("Async Jobs"),
            fill=COLOR_DATASTORE_NAVY, fill_dark=COLOR_DATASTORE_NAVY_DARK,
            fontColor_dark="#0a2540", fontSize=12, bold=False)
# Analytics sink as future state — grey/dashed, brief context sentence.
analytics_future = box(920, 472, 290, 84,
                       "<b>Analytics Sink</b><br>"
                       + sub("Reporting Warehouse", size=10) + "<br>"
                       + desc("Future: out-of-scope reporting pipeline "
                              "for v1.", size=10),
                       fill="#e0e0e0", stroke=COLOR_FUTURE_GREY,
                       fontColor="#616161", fontSize=11, bold=False,
                       dashed=True)
...
edge(browser, api_gw,
     ...
     color=COLOR_PRIMARY_BLUE, color_dark=COLOR_PRIMARY_BLUE_DARK,
     label="HTTPS")

# 2. CDN (future) -> Browser — grey/dashed, no label.
edge(cdn_future, browser,
     exitX=0.5, exitY=0, entryX=0.5, entryY=1,
     color=COLOR_FUTURE_GREY, style="dashed")

# 3. Config -> API Gateway (source = gold). Dashed = configuration mount.
#    Route up the sub-channel at x=372 (gap between External & App).
edge(config, api_gw,
     exitX=1, exitY=0.05, entryX=0, entryY=0.75,
     color=COLOR_CONFIG_GOLD, color_dark=COLOR_CONFIG_GOLD_DARK,
     style="dashed", label="Load Config",
...
edge(api_gw, auth,
     exitX=1, exitY=0.5, entryX=0, entryY=0.5,
     color=COLOR_GATEWAY_ORANGE, label="Verify<div>Token</div>")
...
edge(api_gw, web,
     ...
     color=COLOR_GATEWAY_ORANGE, width=3,
     label="Route<div>Request</div>", label_x=30, label_y=0)
...
edge(web, cache,
     exitX=0.45, exitY=0, entryX=0, entryY=0.5,
     color=COLOR_FRONTEND_GREEN, style="dotted", jump=True, label="Read",
...
edge(web, primary_db,
     exitX=0.55, exitY=0, entryX=0, entryY=0.6,
     color=COLOR_FRONTEND_GREEN, width=3, bidirectional=True,
     label="Read/Write",
     waypoints=[(520, 284), (892, 284), (892, 202)],
     label_x=0, label_y=-20)

# 8. Web -> Message Queue (source = green). Lane y=425 (below the row),
#    sub-channel x=896, enter queue at y=401.
edge(web, queue,
     exitX=0.5, exitY=1, entryX=0, entryY=0.4,
     color=COLOR_FRONTEND_GREEN, label="Enqueue Job",
...
edge(worker, queue,
     ...
     color=COLOR_ORCH_PURPLE, color_dark=COLOR_ORCH_PURPLE_DARK,
     style="dotted", label="Consume",
...
edge(worker, primary_db,
     exitX=0.5, exitY=0, entryX=0, entryY=0.72,
     color=COLOR_ORCH_PURPLE, color_dark=COLOR_ORCH_PURPLE_DARK,
     label="Write Result",
     waypoints=[(755, 272), (888, 272), (888, 215)],
     label_x=0, label_y=-14)
...
edge(primary_db, analytics_future,
     exitX=1, exitY=0.9, entryX=1, entryY=0.5,
     color=COLOR_DATASTORE_NAVY, color_dark=COLOR_DATASTORE_NAVY_DARK,
     style="dashed", label="Replicate",
...
box(40, 616, W - 80, 28, legend_text,
    fill="#ffffff", stroke="#cccccc", fontColor="#444444",
    fill_dark="#1a1a1a", stroke_dark="#555555", fontColor_dark="#cccccc",
    fontSize=10, bold=False, valign="middle", halign="left",
    spacingLeft=10)
